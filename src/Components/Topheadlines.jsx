...
const Topheadlines = () => {
  ...
  const renderCard = () =>
    Array.from({ length: 20 }).map((_, i) => (
      <div key={i} className="p-5 mt-8 rounded-lg bg-white shadow-inner shadow-gray-200 flex items-start gap-4">
            ...
            <div className="h-[600px] scrollbar scrollbar-thumb-[#7E3AF2] scrollbar-sky-300 overflow-y-scroll">
              {isLoading ? (
                ...
                Headlines.map((item, idx) => {
                  // Format date/time
                  const formattedDate = item.dateCrawled
                    ? new Date(item.dateCrawled)
                        .toLocaleString('en-IN', {
                          ...
                          hour: '2-digit',
                          minute: '2-digit',
                          hour12: false, // use 24-hour format (set true for AM/PM)
                        })
                        .replace(',', ' |')
                    : '';
                  ...
                  return (
                    <div
                      key={idx}
                      className="max-w-full h-[120px] p-5 mt-8 rounded-[8px] flex justify-between bg-[#FFFFFF] shadow-inner shadow-gray-200"
                    >
                      <div className="w-[454px] h-[80px] grid grid-cols-[20%_auto] items-center gap-6">
                        <div className="aspect-[4/3] w-[80px]">
                          <img
                            src={item.imageUrl && item.imageUrl !== 'no value' ? item.imageUrl : noimg}
                            alt="image"
                        ...
                        <div>
                          ...
                          <a className="text-md font-semibold break-words line-clamp-2" href={item.url} target="_blank" rel="noreferrer">
                            {item.headline}
                          </a>
